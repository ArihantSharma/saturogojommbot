...
from API.mangahubio import mangahubio as manga
from config import bot


class Manga:
    @bot.on(events.NewMessage(pattern=r"^/manga|^/manga@saturogojommbot"))
    async def event_handler_manga(event):
        if "/manga" == event.raw_text:
            ...
            results = manga.get_search_results(manga_name)
            if len(results) == 0:
                await bot.send_message(
                    event.chat_id,
    ...
    async def event_handler_manga(event):
        try:
            ...
            chap = manga.get_manga_chapter(split_data[0], split_data[1])
    ...
    async def callback_for_mangadets(event):
        ...
        dets = manga.get_manga_details(data[4:])
